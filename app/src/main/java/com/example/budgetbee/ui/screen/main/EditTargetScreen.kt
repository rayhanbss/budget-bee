...
fun EditTargetScreen (
    ...
    val name = remember { mutableStateOf(target.name) }
    val amountNeeded = remember { mutableDoubleStateOf(target.amountNeeded) }
    val deadline = remember { mutableStateOf(target.deadline) }
    ...
    val isTargetUpdated = targetViewModel.isTargetUpdated

    LaunchedEffect(isTargetUpdated) {
        if (targetViewModel.isTargetUpdated) {
            navController.navigate("target")
            targetViewModel.isTargetUpdated = false
        }
    }
    ...
    Column(
        ...
        Column(
            ...
            OutlinedTextField(
                value = amountNeeded.doubleValue.toString(),
                onValueChange = { amountNeeded.doubleValue = it.toDoubleOrNull() ?: 0.0 },
        ...
        Button(
            onClick = {
                targetViewModel.updateTarget(
                    ...
                    targetId = target.id,
                    name = name.value,
                    amountNeeded = amountNeeded.doubleValue,
            ...
            Icon(
                ...
                contentDescription = "Update Target"
            )
            Text(
                text = "Update Target",
                fontSize = 16.sp,
